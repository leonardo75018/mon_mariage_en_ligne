...
class ImageControllers {
    ...
    static async ImagesClient(req, res) {
        try {
            const allimages = await database.image.findAll({ where: { idClient: req.userId } })
            return res.status(200).json(allimages)
        } catch (err) { return res.status(500).json(err.message) }
    }
    ...
    static async actualiserImage(req, res) {
        ...
        try {
            ...
            const imageActualiser = await database.client.findOne({ where: { id: Number(id) } })
    ...
    static async deleteImage(req, res) {
        const { id } = req.params
        try {
            await database.client.destroy({ where: { id: Number(id) } })
